...
//! Specify region flags
            enum RegionLocationFlag {
                REGION_FLAG_FUNCTION = (1
                        << 0),             //< region is function (=1) / nested named region (=0)
                REGION_FLAG_APP_CODE = (1
                        << 1),             //< region is Application code (=1) / OpenCV library code (=0)
                ...
                REGION_FLAG_REGION_NEXT = (1
                        << 31),         //< close previous region (see #CV_TRACE_REGION_NEXT macro)
