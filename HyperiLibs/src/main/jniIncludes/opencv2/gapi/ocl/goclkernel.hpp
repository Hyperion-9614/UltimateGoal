...
// NEED TO FIX: This is an ugly ad-hoc implementation. NEED TO FIX: refactor
    ...
    namespace detail {
        ...
        struct ocl_get_in<cv::GArray<U> > {
            static const std::vector <U> &
            get(GOCLContext &ctx, int idx) { return ctx.inArg<VectorRef>(idx).rref<U>(); }
        };
...
class GOCLKernelImpl
        : public cv::detail::OCLCallHelper<Impl, typename K::InArgs, typename K::OutArgs>,
