...
<
GMat(GMat,
double,double,int,bool)>, "org.opencv.imgproc.canny"){
...
sepFilter(const GMat &src, int ddepth, const Mat &kernelX, const Mat &kernelY,
          const Point &anchor /*NEED TO FIX: = Point(-1,-1)*/,
          const Scalar &delta /*NEED TO FIX = GScalar(0)*/, int borderType = BORDER_DEFAULT,
