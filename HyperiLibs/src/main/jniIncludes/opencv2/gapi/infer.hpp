...
// NEED TO FIX: Inference API is currently only available in full mode
...
// NEED TO FIX: maybe tuple_wrap_helper from util.hpp may help with this.
...
struct GInfer final
        : public GInferBase, public detail::KernelTypeMedium<GInfer<Net>, typename Net::API> {
    using GInferBase::getOutMeta; // NEED TO FIX: name lookup conflict workaround?
...
struct GInferList final
    ...
    using GInferListBase::getOutMeta; // NEED TO FIX: name lookup conflict workaround?
...
struct GInferList2 final
        : public GInferList2Base,
          public detail::KernelTypeMedium<GInferList2<Net, Args...>, typename InferAPIList2<Net, Args...>::type> {
    using GInferList2Base::getOutMeta; // NEED TO FIX: name lookup conflict workaround?
...
// NEED TO FIX: Probably the <API> signature makes a function/tuple/function round-trip
 ...
 */
        ...
        typename Net::ResultL infer2(cv::GMat image, cv::GArray<Args>... args) {
            // NEED TO FIX: Declared as "2" because in the current form it steals
...
// Similar to how a type-erased GKernel is represented and used.
        struct GAPI_EXPORTS GNetParam{
                std::string tag;     // NEED TO FIX: const?
