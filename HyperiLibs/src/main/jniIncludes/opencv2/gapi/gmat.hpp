...
struct GAPI_EXPORTS GMatDesc
        {
                // NEED TO FIX: Default initializers in C++14
                int depth;
        int chan;
        cv::Size size; // NB.: no multi-dimensional cases covered yet
        bool planar;
        std::vector<int> dims; // NEED TO FIX: Maybe it's real questionable to have it here
...
GAPI_EXPORTS GMatDesc

descr_of(const cv::UMat &mat);
...
namespace gapi {
    namespace own {
        ...
        descr_of(const Mat &mat);
    }
}//gapi::own

GAPI_EXPORTS GMatDesc

descr_of(const RMat &mat);

#if !defined(GAPI_STANDALONE)
GAPI_EXPORTS GMatDesc

descr_of(const cv::Mat &mat);
