...
// NEED TO FIX: put into "details" as a user shouldn't use it in his code
...
// NEED TO FIX: Express as M<GProtoArg...>::type
 ...
 */
    ...
    namespace gapi {
        GAPI_EXPORTS cv::GRunArgsP
        bind(cv::GRunArgs
        &results);
        GAPI_EXPORTS cv::GRunArg
        bind(cv::GRunArgP
        &out);     // NEED TO FIX: think more about it
    }

    template<typename... Ts>
    inline GRunArgs gin(const Ts &... args) {
        return GRunArgs{GRunArg(detail::wrap_host_helper<Ts>::wrap_in(args))...};
    }

    template<typename... Ts>
    inline GRunArgsP gout(Ts &... args) {
