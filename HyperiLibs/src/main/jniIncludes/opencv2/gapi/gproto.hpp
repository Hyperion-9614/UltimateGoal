...
// directly.
    using GProtoArg = util::variant
            <GMat, GMatP, GFrame, GScalar, detail::GArrayU  // instead of GArray<T>
    ...
    namespace detail {
        ...
        inline GProtoArgs packArgs(Ts... args) {
...
// Perfect forwarding
    template<typename... Ts>
    inline GProtoInputArgs GIn(Ts &&... ts) {
        return GProtoInputArgs(detail::packArgs(std::forward<Ts>(ts)...));
    }

    template<typename... Ts>
    inline GProtoOutputArgs GOut(Ts &&... ts) {
...
// extracted from that arguments
    GMetaArg GAPI_EXPORTS

    descr_of(const GRunArg &arg);

    GMetaArgs GAPI_EXPORTS

    descr_of(const GRunArgs &args);
...
// Used to compare the metadata, which generated at compile time with the metadata result operation in run time
    ...
    descr_of(const GRunArgP &argp);

// Checks if run-time computation argument can be described by metadata
    bool GAPI_EXPORTS

    can_describe(const GMetaArg &meta, const GRunArg &arg);

    bool GAPI_EXPORTS

    can_describe(const GMetaArgs &metas, const GRunArgs &args);
...
// coincides with output arguments passed to computation in cpu and ocl backends
    ...
    can_describe(const GMetaArg &meta, const GRunArgP &argp);

// Validates input arguments
    void GAPI_EXPORTS

    validate_input_arg(const GRunArg &arg);

    void GAPI_EXPORTS

    validate_input_args(const GRunArgs &args);
