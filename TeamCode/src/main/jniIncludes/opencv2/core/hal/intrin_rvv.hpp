...
CV__HAL_INTRIN_EXPAND_WITH_FP_TYPES(CV__HAL_INTRIN_IMPL_BIT_OP_, bit_op) /* NEED TO FIX: FIXIT remove this after masks refactoring */
...
CV_INLINE v_reg<double, 2>

v_cvt_f64(const v_reg<int, 4> &a) {
    enum {
        n = 2
    };
    v_reg<double, n> c;
...
v_cvt_f64_high(const v_reg<int, 4> &a) {
    enum {
        n = 2
    };
    v_reg<double, n> c;
...
v_cvt_f64(const v_reg<float, 4> &a) {
    enum {
        n = 2
    };
    v_reg<double, n> c;
...
v_cvt_f64_high(const v_reg<float, 4> &a) {
    enum {
        n = 2
    };
    v_reg<double, n> c;
...
v_cvt_f64(const v_reg<int64, 2> &a) {
    enum {
        n = 2
    };
    v_reg<double, n> c;
...
v_cvt_f64_high(const v_reg<int64, 2> &a) {
    enum {
        n = 2
    };
    v_reg<double, n> c;
...
template<typename _Tp>
inline void v_transpose4x4(v_reg<_Tp, 4> & a0,
