...
namespace cv {
    namespace detail {
        ...
        struct GTypeTraits<cv::GArray<T> > {
            ...
            static cv::detail::VectorRef
            wrap_in(const std::vector <T> &t) { return detail::VectorRef(t); }
        ...
        struct has_custom_wrap {
            ...
            static const constexpr bool value = std::is_same<std::true_type, decltype(test<T>(
                    nullptr))>::value;
        };
    ...
    struct WrapValue {
        ...
        template<typename U>
        static U wrap_in(const U &u) { return u; }

        template<typename U>
        static U *wrap_out(U &u) { return &u; }
    };
