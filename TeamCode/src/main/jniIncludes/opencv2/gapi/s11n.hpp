...
namespace cv {
    namespace gapi {
        ...
        namespace detail {
            GAPI_EXPORTS cv::GComputation

            getGraph(const std::vector<char> &p);

            GAPI_EXPORTS cv::GMetaArgs

            getMetaArgs(const std::vector<char> &p);

            GAPI_EXPORTS cv::GRunArgs
        ...
        serialize(const cv::GComputation &c);
//namespace{
...
//} //ananymous namespace
        ...
        serialize(const cv::GCompileArgs &);

        GAPI_EXPORTS std::vector<char>

        serialize(const cv::GMetaArgs &);
...
namespace cv {
    namespace gapi {
        namespace s11n {
            ...
            namespace detail {
                GAPI_EXPORTS std::unique_ptr<IIStream>

                getInStream(const std::vector<char> &p);
            } // namespace detail
