            ...
            namespace detail {
                ...
                struct MatHeader {
                    ...
                    MatHeader(int _rows, int _cols, int type, void *_data, size_t _step)
                            : flags((type & TYPE_MASK)), rows(_rows), cols(_cols),
                              data((uchar *) _data),
                              step(_step == AUTO_STEP ? detail::default_step(type, _cols)
                                                      : _step) {}

                    MatHeader(const std::vector<int> &_dims, int type, void *_data)
                            : flags((type & TYPE_MASK)), data((uchar *) _data), step(0),
                              dims(_dims) {}
                ...
                */
                void create(Size _size, int _type) {
                    ...
                    if (_size != Size{cols, rows}) {
                        ...
                        tmp.memory.reset(new uchar[tmp.step * tmp.rows],
                                         [](uchar *p) { delete[] p; });
                ...
                size_t total() const {
                    return dims.empty()
                           ...
                           : std::accumulate(dims.begin(), dims.end(), static_cast<std::size_t>(1),
