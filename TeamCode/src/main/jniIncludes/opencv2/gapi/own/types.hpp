    ...
    namespace gapi {
        namespace own {
            ...
            public:
                ...
                Rect(int _x, int _y, int _width, int _height) : x(_x), y(_y), width(_width),
                                                                height(_height) {};
#if !defined(GAPI_STANDALONE)

                Rect(const cv::Rect &other) : x(other.x), y(other.y), width(other.width),
                                              height(other.height) {};
            ...
            inline std::ostream &operator<<(std::ostream &o, const Rect &rect) {
                return o << "[" << rect.width << " x " << rect.height << " from (" << rect.x << ", "
                         << rect.y << ")]";
            }
